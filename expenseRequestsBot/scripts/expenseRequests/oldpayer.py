...
def get_payer_requests():
    ...
    try:
        cursor.execute("""
            ...
            WHERE status = 'Одобрено финансовым отделом, передано на оплату'
        """)
    ...
    if data == 'next_request':
        ...
        if data.startswith('payer_paid_'):
            ...
            try:
                ...
                conn.close()
                if request_data:
                    send_webhook(request_data)
            except Exception as e:
                logger.error(f"Ошибка при отправке webhook: {e}")

            # Ищем транзакцию в Tron
            txn_link = None
            if request_data:
                try:
                    to_address = request_data[6]  # details
                    raw_amount = request_data[4]  # amount
                    txn_link = find_tron_transaction_in_last_day(raw_amount, to_address)
                except Exception as ex:
                    logger.warning(f"Ошибка при поиске транзакции Tron: {ex}")
                    txn_link = None
            ...
            try:
                ...
                if request:
                    ...
                    if txn_link:
                        notification_text += f"\n<a href='{txn_link}'>Ссылка на транзакцию</a>"
